...
运行：python -m standalone.web   （默认 http://127.0.0.1:8000，可经 .env 的 WEB_HOST/WEB_PORT 改）
...
from runtime.world_json import world_to_json, load_world_from_json
from .agent import make_agent
from .config import LLMConfig

_HERE = Path(__file__).parent
_INDEX = _HERE / "web" / "index.html"
_EDITOR = _HERE / "web" / "editor.html"

# 一键试玩：编辑器请求试玩某世界时设这个，下一次空开局据此 start_game 该世界（而非默认 aincrad）。
_NEXT_OPENING_WORLD: str | None = None
...
def _situation_line() -> str:
    """据真实初始状态拼一句通用开场处境（血量/有无武器/有无钱）——非 aincrad 世界用，
    避免把'破局/赤贫'这种 aincrad 专属设定套到别的世界上。"""
    st = mcp_server.SESSION.state
    if not st:
        return ""
    v = st.vitals
    bits = [f"你有 {v.hp}/{v.max_hp} 点血"]
    has_weapon = any(getattr(it, "equip_slot", "") == "weapon" for it in st.inventory)
    if not st.inventory:
        bits.append("身无长物")
    elif not has_weapon:
        bits.append("手无寸铁")
    if v.gold <= 0:
        bits.append("身无分文")
    return "开场处境：" + "、".join(bits) + "。"


def _opening_seed(world: str | None = None) -> str:
    """全新开局：服务端先把游戏 start_game 起来，再把权威开场信息打包成给 GM 的种子，
    强制它【只依据真实场景】写开场，杜绝"不调工具、凭空脑补开场"。

    world 省略时用一键试玩指定的世界（_NEXT_OPENING_WORLD），再回退默认 aincrad。"""
    global _NEXT_OPENING_WORLD
    wname = world or _NEXT_OPENING_WORLD or "aincrad"
    _NEXT_OPENING_WORLD = None
    try:
        sg = mcp_server.start_game(wname)
    except Exception:
        return ""
    if not sg.get("ok"):
        return ""
    ...
    if isinstance(canon, dict):
        ...
        blurb = canon[:400]
    # aincrad 的"破局/赤贫"开场是精调过的专属设定；其它世界据真实初始状态通用生成。
    situation = ("开场处境：你只有 6 点血、赤手空拳、身无分文、谁也不认识——这是一个『破局』开局。"
                 if wname == "aincrad" else _situation_line())
    return (
...
# ── 世界编辑器端点 ─────────────────────────────────────────────────
# 把世界当数据增删改：列世界 / 读世界 JSON / 实时校验 / 保存到 worlds/*.json / 一键试玩。
_NAME_RE = re.compile(r"^[\w一-鿿-]{1,40}$")   # 安全世界名：词字符/中文/连字符，防路径穿越


def _safe_world_name(name: str) -> str | None:
    name = (name or "").strip()
    return name if _NAME_RE.match(name) else None


def _world_to_editable_json(name: str) -> dict | None:
    """取某世界的可编辑 JSON：JSON 世界直接给存盘数据；内置（Python）世界导出一份供查看/另存。"""
    mod = mcp_server.WORLDS.get(name)
    if mod is None:
        return None
    if hasattr(mod, "data"):                      # JsonWorld：直接回存盘数据
        return dict(mod.data, name=name)
    return world_to_json(GameWorld(content_module=mod), name)   # 内置：导出快照


@app.get("/editor", response_class=HTMLResponse)
def editor_page() -> str:
    if _EDITOR.exists():
        return _EDITOR.read_text(encoding="utf-8")
    return "<h1>编辑器文件缺失：standalone/web/editor.html</h1>"


@app.get("/editor/worlds")
def editor_worlds() -> JSONResponse:
    """列出所有世界：名字 + 是否可编辑（JSON 世界可改，内置只读可另存）+ 加载错误。"""
    out = []
    for name, mod in mcp_server.WORLDS.items():
        out.append({"name": name, "editable": hasattr(mod, "data"),
                    "builtin": not hasattr(mod, "data")})
    out.sort(key=lambda w: (not w["builtin"], w["name"]))
    return JSONResponse({"worlds": out, "errors": mcp_server.WORLD_LOAD_ERRORS})


@app.get("/editor/world/{name}")
def editor_get_world(name: str) -> JSONResponse:
    data = _world_to_editable_json(name)
    if data is None:
        return JSONResponse({"ok": False, "error": f"未知世界 {name!r}"}, status_code=404)
    return JSONResponse({"ok": True, "world": data,
                         "editable": hasattr(mcp_server.WORLDS.get(name), "data")})


@app.post("/editor/validate")
async def editor_validate(request: Request) -> JSONResponse:
    """实时校验：拿前端传来的世界 JSON 构建 GameWorld 跑 validate()，不落盘。"""
    data = await request.json()
    try:
        problems = load_world_from_json(data).validate()
    except Exception as e:
        return JSONResponse({"ok": False, "error": f"{type(e).__name__}: {e}"})
    return JSONResponse({"ok": True, "problems": problems})


@app.post("/editor/world/{name}")
async def editor_save_world(name: str, request: Request) -> JSONResponse:
    """保存世界到 worlds/{name}.json，并热刷新 WORLDS。返回校验问题列表（不阻止保存）。"""
    ...
    data["name"] = safe
    try:
        problems = load_world_from_json(data).validate()   # 先确认能构建（结构合法）
    except Exception as e:
        return JSONResponse({"ok": False, "error": f"无法构建该世界：{type(e).__name__}: {e}"})
    path = mcp_server.WORLDS_DIR / f"{safe}.json"
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    mcp_server.load_json_worlds()
    return JSONResponse({"ok": True, "name": safe, "problems": problems})


@app.delete("/editor/world/{name}")
def editor_delete_world(name: str) -> JSONResponse:
    safe = _safe_world_name(name)
    path = mcp_server.WORLDS_DIR / f"{safe}.json" if safe else None
    if not safe or not path.exists():
        return JSONResponse({"ok": False, "error": "该世界不是可删除的 JSON 世界。"})
    path.unlink()
    mcp_server.load_json_worlds()
    return JSONResponse({"ok": True})


@app.post("/editor/playtest/{name}")
async def editor_playtest(name: str) -> JSONResponse:
    """一键试玩：清当前会话，置下次开局世界为 name，前端随后跳到 / 起手。"""
    global _agent, _NEXT_OPENING_WORLD
    if name not in mcp_server.WORLDS:
        return JSONResponse({"ok": False, "error": f"未知世界 {name!r}（先保存）。"})
    with _lock:
        for p in (mcp_server.SAVE_DIR / f"{_CURRENT_SLOT}.json", _session_path(_CURRENT_SLOT)):
            try:
                p.unlink(missing_ok=True)
                ...
                pass
        mcp_server.SESSION = mcp_server.Session()
        _agent = make_agent(LLMConfig.from_env(), rich_ui=True)
        _NEXT_OPENING_WORLD = name
    return JSONResponse({"ok": True, "play_url": "/"})


def main() -> int:
    cfg = LLMConfig.from_env()      # 顺带加载 .env，下面读 WEB_HOST/WEB_PORT 才生效
    err = cfg.validate()
    if err:
    ...
    try:
        ...
        port = 8000
    import uvicorn
    print(f"文字冒险 Web 服务：http://{host}:{port}")
