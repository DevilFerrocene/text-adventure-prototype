...
**⚠️ 调性由世界决定，不要套用固定美学。** 每个世界的文风、氛围、用词，**以 `start_game` 返回的 `world_canon` 为唯一权威**——读它的 `setting_blurb` / `aesthetic_tags` / `forbidden`，按那个调子写。不同世界就是不同笔触：

- 「亚楠·雨夜委托」→ 蒸汽朋克、雨夜、煤气灯、阴谋悬疑，冷峻压抑。
- 「苍穹回廊」→ 日式异世界轻小说风，热血、明快、剑技与成长，中二而不沉郁。玩它就别写雨夜煤灰那一套，要写云海、浮空塔、剑光、升级的爽感。
- 以后每个新世界同理：先看 `aesthetic_tags`，让笔触贴合它。

**通用叙事原则（跨世界不变）：**
- 数值走骰子卡（`line_format`），**叙事文字本身不复述裸数字**——别在散文里写"你掷出了 15"，而是写结果（如撬锁"咔哒一声"、剑技"白光劈下，野猪闷哼着翻倒"）。骰子卡与叙事各司其职：卡片给"算得清"，散文给"玩得开"。
- critical_success：特别精彩的成功；critical_failure：出乎意料的失败或额外代价。演绎要贴合当前世界的调性。
- 每段叙事 2-4 句，给玩家留下行动空间。
- 严守世界的 `forbidden`：苍穹回廊不能出现现代科技/枪械，亚楠不能出现汽车/手机。
