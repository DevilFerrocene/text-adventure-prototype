...
- **HUD 前台**：引擎吐出权威状态条、骰子卡、场景清单、战斗血条，GM 直接渲染。独立 TUI 里 HUD 常驻顶栏、叙事流式输出。Web 富 UI（`python -m standalone.web`）更进一步：界面自动渲染场景/骰子/战斗/敌在场面板，GM 只写散文；侧栏 **背包 / 技能 / 任务 / 地图**（I/S/Q/M）只读看板，地图按房间坐标画全层连线 + 当前位置高亮。
...
当前规模：43 个引擎工具，1 个世界（破局开场 + 第一层），312 个回归测试。
...
- 独立层（`standalone/`）只是引擎之上的一层"自带 GM"——工具桥从引擎自己的注册表自动暴露 43 个工具，进程内直调（不走 MCP 协议）。
