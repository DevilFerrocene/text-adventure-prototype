...
- **敌人在棋盘上 · 视野与仇恨**：刷到的怪也落在棋盘格上，带 `sight`（视野）与仇恨状态，**默认摆在你视野外**——你隔着野地先瞥见它（`scene.grid` 给方位/距离 + 「未察觉/已锁定」）。**走进它视野（切比雪夫 ≤ sight，且墙体用 Bresenham 遮挡判定）就被发现、自动进战**；反之只要不踏进视野就能潜行绕过。狼视野广（难溜），啃草的兔迟钝（能贴近）。战斗结算仍走 rank/reach。
...
当前规模：46 个引擎工具，1 个世界（破局开场 + 第一层），391 个回归测试。
