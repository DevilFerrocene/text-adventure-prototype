...
- **两个示例世界**：「亚楠·雨夜委托」（蒸汽朋克暗杀悬疑，正面战斗/潜行绞杀/嫁祸谋略三解法）+「苍穹回廊」（日式异世界逐层攻略，剑技/属性克制/成长）。

当前规模：40 个引擎工具，2 个世界（各含完整主线），206 个回归测试。
...
content/yanan.py       # 示例世界「亚楠·雨夜委托」
content/aincrad.py     # 示例世界「苍穹回廊」
...
content 是纯数据。照着 `content/yanan.py` 或 `content/aincrad.py` 写一份新模块——房间 `Room`、物体 `GameObject`、敌人 `EnemyTemplate`、技能 `Skill`、任务 `QuestEntry`，可选一份 `RuleBook`（自定义该世界的属性表/装备槽，不写就用默认四属性）——然后在 `mcp_server.py` 的 `WORLDS` 字典里注册即可。物体的 `on_destroyed`、affordance 的 `effect`、武器的 `scaling` 都复用同一套数据 DSL，无需改引擎。
