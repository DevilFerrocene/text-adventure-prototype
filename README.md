...
当前规模：41 个引擎工具，1 个世界（第一层完整可玩），206 个回归测试。
...
- 独立层（`standalone/`）只是引擎之上的一层"自带 GM"——工具桥从引擎自己的注册表自动暴露 41 个工具，进程内直调（不走 MCP 协议）。
