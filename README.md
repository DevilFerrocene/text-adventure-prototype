...
- **二维棋盘（房间空间感）**：房间可选挂一块 `RoomGrid`——物体/陈设/出口钉在格子上，玩家是棋盘上一枚 token。**引擎独占坐标与寻路（BFS），对外只吐方位（8 向）+ 距离（步/远近词）**，省 token、防 GM 编坐标。`get_scene` 把"手边可交互"与"四周得走过去"分开，叙事自然有了前景/背景纵深；`approach(target)` 走位、够不着的东西交互时引擎自动寻路过去。没挂棋盘的房间隐式"满屋皆在手边"（向后兼容）。
- **敌人在棋盘上 · 视野与仇恨**：刷到的怪也落在棋盘格上，带 `sight`（视野）与仇恨状态，**默认摆在你视野外**——你隔着野地先瞥见它（`scene.grid` 给方位/距离 + 「未察觉/已锁定」）。**走进它视野（切比雪夫 ≤ sight，且墙体用 Bresenham 遮挡判定）就被发现、自动进战**；反之只要不踏进视野就能潜行绕过。狼视野广（难溜），啃草的兔迟钝（能贴近）。
- **棋盘战斗（统一空间模型）**：野外撞见/伏击/搜出来的战斗**直接在这张棋盘上打**——combatant 有 `cell` 与按速度派生的 `move_range`，**触及=格距**（近战须相邻、长柄 2、远程大且查视线），`move toward:<id>/away:<id>/方位` 走格、离开近战触及吃借机攻击。**敌人位置 AI**：近战寻路逼近、远程被贴脸就放风筝、多怪自然包抄。剧本预设遭遇（首领/酒馆）仍走抽象列阵 rank。探索与战斗终于一套坐标。
- **探索点（未探明的「?」）**：棋盘上可埋 `GridPOI`——明面只有一句不剧透的感官提示（"草叶间反着点光"），**谜底锁在引擎里、揭示前 GM 也看不到是啥**（引擎硬版的"别剧透"）。玩家 `approach` 走到跟前**自动揭示**：战利品 / 线索 / 事件 / 陷阱（引擎掷 save 扣血）/ 伏击（就地把敌人写进在场、随后开打）。一次性、进存档；前端「场地」面板画成紫色虚线 `?` 格。
...
- **HUD 前台**：引擎吐出权威状态条、骰子卡、场景清单、战斗血条，GM 直接渲染。独立 TUI 里 HUD 常驻顶栏、叙事流式输出。Web 富 UI（`python -m standalone.web`）更进一步：界面自动渲染场景/骰子/战斗/敌在场面板，GM 只写散文；侧栏 **背包 / 技能 / 任务 / 地图 / 场地**（I/S/Q/M/B）只读看板，地图按房间坐标画全层连线 + 当前位置高亮，**场地**把当前房间的二维棋盘画成 token 板（物件/陈设/出口分色、「你」实时高亮）。
...
当前规模：46 个引擎工具，1 个世界（破局开场 + 第一层），400 个回归测试。
